<div ng-if="question_type=='checkbox'">
    ...
    <fieldset class="form-group col-md-4" >
        <label >Вариант @{{ $index+1 }}</label> <span class="text-red">*</span>
        <input type="text" name="options[]" id="option_@{{ $index }}" class="form-control" placeholder="Вариант @{{ $index+1 }}" ng-model="answers[$index].option_value" required="true">
    </fieldset>

     {{-- <fieldset class="form-group col-md-4" >
        
        <label >Вариант @{{ $index+1 }} (2й язык) </label> 
        <input type="text" name="optionsl2[]" id="optionl2_@{{ $index }}" class="form-control" placeholder="Вариант 2й язык @{{ $index+1 }}" ng-model="answers[$index].optionl2_value">
