...
<div ng-if="question_type=='para' || question_type=='video' || question_type=='audio'">
    ...
    <div class="col-md-12 question-block">
     ...
     <fieldset class="form-group" >
        ...
        <span class="text-red">*</span>
        <input type="text" name="questions_list[]" id="question_@{{ $index }}" class="form-control" placeholder="Вопрос @{{ $index+1 }}" ng-model="answers[$index].question"  required="true">
    ...
    <fieldset class="form-group col-md-6" ng-repeat="i in range(total_para_options) track by $index " ng-if="total_para_options >0 " >
        ...
        <span class="text-red">*</span>
        <input type="text" name="options[@{{ para }}][]" id="option_@{{ $index }}" class="form-control" placeholder="Вариант @{{ $index+1 }}" ng-model="answers[para]['options'][para][$index]" required="true">
    ...
    <fieldset class="form-group col-md-12" >
         ...
         <span class="text-red">*</span>
             <input type="text" name="correct_answers[]" id="correctanswer_@{{ $index }}" class="form-control" placeholder="Ответ @{{ $index+1 }}" ng-model="correct_answers[para].answer" required="true" min="1">
...
{{-- 
    ...
    <div class="col-md-12 question-block">
     ...
     <fieldset class="form-group" >
        ...
        <label >Вопрос @{{ $index+1 }} (2й язык)</label>
        
        <input type="text" name="questions_listl2[]" id="questionl2_@{{ $index }}" class="form-control" placeholder="Вопрос @{{ $index+1 }}" ng-model="answers[$index].questionl2">
    ...
    <fieldset class="form-group col-md-6" ng-repeat="i in range(total_para_options) track by $index " ng-if="total_para_options >0 " >
        <label >Вариант @{{ $index+1 }} (2й язык)</label> 
        
        <input type="text" name="optionsl2[@{{ para }}][]" id="optionl2_@{{ $index }}" class="form-control" placeholder="Вариант @{{ $index+1 }}" ng-model="answers[para]['optionsl2'][para][$index]">
