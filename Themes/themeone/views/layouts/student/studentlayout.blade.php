	...
	<div id="wrapper" class="{{$class}}">
		...
		<?php } ?>
			...
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="/"><img src="{{IMAGE_PATH_SETTINGS.getSetting('site_logo', 'site_settings')}}" alt="{{getSetting('site_title','site_settings')}}"></a>
				...
				<ul class="nav navbar-nav side-nav {{$block_class}}">
					...
					@if(!checkRole(getUserGrade(5)))
						...
						<li>
							...
							<a href="{{URL_PARENT_CHILDREN}}"><i class="fa fa-user-circle" ></i>
...
{{--						<li><a href="{{ URL_STUDENT_EXAM_ALL }}"> <i class="fa fa-list-ol"></i>{{ getPhrase('exam_series') }}</a></li>--}}
...
{{--					</li>--}}
					...
					<ul id="analysis" class="collapse sidemenu-dropdown">
						...
						<li><a href="{{URL_STUDENT_ANALYSIS_BY_EXAM.Auth::user()->slug }}"> <i class="fa fa-suitcase"></i>Опросники</a></li>
						<li><a href="/analysis/lms-points/{{Auth::user()->slug}}"> <i class="fa fa-video-camera"></i>Видео</a></li>
						<li><a href="/analysis/all-points/{{Auth::user()->slug}}"> <i class="fa fa-bookmark"></i>Единицы</a></li>
    ...
    <script>
			...
			player.on('play', event => {
				$.ajax({
					url: '/points',
					type: 'POST',
					data: {_token :csrfToken, 'slug': slug, 'category': category},
    		...
    		player.on('timeupdate', event => {
				var timer = $('.plyr__controls').find('input');
				var time = $(timer).attr('aria-valuetext');
				$.ajax({
					url: '/update-timer',
					type: 'POST',
					data: {_token :csrfToken, 'slug': slug, 'time': time},
					// success: function (data) {
					// 	console.log(data);
					// }
				});
