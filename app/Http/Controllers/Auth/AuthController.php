    ...
    */
    ...
    public function socialLoginCancelled(Request $request)
    {
     ...
     public function resetUsersPassword(Request $request)
     {
         ...
         $request->phone_1 = preg_replace('/[^0-9]/','' ,$request->phone_1);
         $user  = User::where('phone','=',$request->phone_1)->first();
         DB::beginTransaction();
         try{
         ...
         if($user!=null){
           ...
           $sms = "Ваш новый пароль";
           $phone = '996'.preg_replace('~[^0-9]+~','',$request->phone_1);

            new \App\Http\SmsSend($phone, $password, $sms);

           session()->flash('success', 'Пароль был отправлен на ваш телефон');

         }

         else{

            session()->flash('error','Этот номер недействителен');
