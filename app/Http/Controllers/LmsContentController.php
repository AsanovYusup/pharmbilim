...
class LmsContentController extends Controller
{
    ...
    $records = LmsContent::join('subjects', 'lmscontents.subject_id', '=', 'subjects.id')
        ...
        ->addColumn('action', function ($records) {
            $extra = '<div class="col text-center"><div class="btn-group">
        <button type="button" class="btn btn-xs btn-success btn-icon rounded waves-effect waves-themed" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
               ...
               <div class="dropdown-menu">
                            <a class="dropdown-item" href="'.URL_LMS_CONTENT_EDIT.$records->slug.'"><i class="fal fa-pencil"></i> '.getPhrase("edit").'</a>';
                            $temp = "";
      ...
      if(!checkRole(getUserGrade(2)))
      {
       ...
       $name = $request->title;
        ...
        if ($request->content_type == 'url') {
            if (substr($request->file_path, 0, 4) !== 'http') {
               $record->file_path = 'http://'.$request->file_path;
            }
        }
        $record->save();
         $file_name = 'image';
