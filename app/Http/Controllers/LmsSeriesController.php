...
class LmsSeriesController extends Controller
{
        ...
        ->addColumn('action', function ($records) {
          ...
          $link_data = '<div class="col text-center"><div class="btn-group">
                      <button type="button" class="btn btn-xs btn-success btn-icon rounded waves-effect waves-themed" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                           ...
                           <a class="dropdown-item" href="'.URL_LMS_SERIES_UPDATE_SERIES.$records->slug.'"><i class="fal fa-spinner"></i> '.getPhrase("update").'</a>
                            <a class="dropdown-item" href="'.URL_LMS_SERIES_EDIT.$records->slug.'"><i class="fal fa-pencil"></i> '.getPhrase("edit").'</a>';
                            
                           $temp = '';
                           if(checkRole(getUserGrade(1))) {
                    $temp .= ' <li><a class="dropdown-item" href="javascript:void(0);" onclick="deleteRecord(\''.$records->slug.'\');"><i class="fal fa-trash"></i> '. getPhrase("delete").'</a></li>';
                      }
    ...
    public function updateSeries($slug)
    {
       ...
       if(!checkRole(getUserGrade(2)))
       {
    	...
    	$data['title']              = getPhrase('update_series_for');
