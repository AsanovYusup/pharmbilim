...
class SiteController extends Controller
{
    ...
    public function index()
    {
      ...
      if (env('DB_DATABASE')!='') {      
        try {          
         ...
         // dd($data['home_back_image']);
         $data['key'] = 'home';
         $data['trainings'] = DB::table('trainings')->get();
    ...
    public function trainings()
    {
        ...
        $record = DB::table('trainings')->whereNotNull('theme')->get();

        $data['record'] = $record;

        $data['layout']      = getLayout();
        $data['active_class'] = 'trainings';
        $data['heading']      = "Тренинги";
        $data['title']        = "Тренинги";

        $view_name = getTheme().'::trainings.trainings';
        return view($view_name, $data);


    }

    public function editTrainings($slug)
    {
        $record = DB::table('trainings')->where('slug', $slug)->first();
        ...
        $view_name = getTheme().'::trainings.trainings-add';
        return view($view_name, $data);

    }

    public function updateTrainings(Request $request, $slug)
    {
        ...
        if ($request->img !== null){
            ...
            $messsages = array(
                ...
                'mimes' => 'Допустимый формат: jpeg,png,jpg',
            ...
            $imageName = $slug.'.'.$request->img->getClientOriginalExtension();

            $request->img->move(public_path('images/trainings'), $imageName);
        }
        ...
        $data = [
            ...
            'img' => $imageName ?? $record->img
        ];

        DB::table('trainings')->where('slug', $slug)->update($data);

        flash('','Ваши данные успешно добавлены', 'success');
        return redirect('/trainings');
    }

    public function deleteTrainings($slug)
    {
        $record = DB::table('trainings')->where('slug', $slug)->first();

        $image_path = public_path('images/trainings/'. $record->img);

        DB::table('trainings')->where('slug', $slug)->delete();
    ...
    public function addTrainings()
    {
        ...
        $data['layout']      = getLayout();
        $data['active_class'] = 'trainings';
        $data['heading']      = 'Добавить тренинг';
        $data['title']        = 'Добавить тренинг';
    ...
    public function partners()
    {
        ...
        $record = DB::table('partners')->get();

        $data['record'] = $record;

        $data['layout']      = getLayout();
        $data['active_class'] = 'partners';
        $data['heading']      = "Партнеры";
        $data['title']        = "Партнеры";

        $view_name = getTheme().'::partners.partners';
        return view($view_name, $data);


    }

    public function editPartners($slug)
    {
        $record = DB::table('partners')->where('slug', $slug)->first();
        ...
        $view_name = getTheme().'::partners.partners-add';
        return view($view_name, $data);

    }

    public function updatePartners(Request $request, $slug)
    {
        ...
        if ($request->img !== null){
            ...
            $request->img->move(public_path('images/partners'), $imageName);
        }
        ...
        DB::table('partners')->where('slug', $slug)->update($data);

        flash('','Ваши данные успешно добавлены', 'success');
        return redirect('/partners');
    }

    public function deletePartners($slug)
    {
        $record = DB::table('partners')->where('slug', $slug)->first();

        $image_path = public_path('images/partners/'. $record->img);

        DB::table('partners')->where('slug', $slug)->delete();
