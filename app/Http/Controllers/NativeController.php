...
class NativeController extends Controller
{
    ...
    public function getDatatable()
    {
    	if(!checkRole(getUserGrade(2)))
      {
        ...
        ->addColumn('action', function ($records) {
           $link_data = '<div class="col text-center"><div class="btn-group">
              ...
              <div class="dropdown-menu">
                  <a class="dropdown-item" href="'.URL_LANGUAGES_UPDATE_STRINGS.$records->slug.'">'.getPhrase("update_strings").'</a>
                  <a class="dropdown-item" href="'.URL_LANGUAGES_EDIT.'/'.$records->slug.'">'.getPhrase("edit").'</a>
        ...
        {
        	 if($records->is_rtl)
            return '<i class="fal fa-check-square text-success" title="'.getPhrase('enable').'"></i>';
            return '<i class="fal fa-times-square text-danger" title="'.getPhrase('disable').'"></i>';
        })
         ->editColumn('is_default',function ($records)
        {
        	 if($records->is_default)
            return '<i class="fal fa-check-square text-success" title="'.getPhrase('enable').'"></i>';
            return '<a href="'.URL_LANGUAGES_MAKE_DEFAULT.$records->slug.'" class="btn btn-xs btn-success waves-effect waves-themed">'.getPhrase('set_default').'</a>';
        })
        ->removeColumn('id')
        ->removeColumn('slug')
        ->make();
