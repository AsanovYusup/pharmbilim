...
class StudentQuizController extends Controller
{
    ...
    public function exams($slug = '')
    {
        ...
        if ($slug) {
            if ($slug != 'all') {
                ...
                if ($role === 'student' || $role === 'parent' || $role === 'parent_company' || $role === 'parent_pharm') {
                    if ($user->settings) {
                        $interested_categories = json_decode($user->settings)->user_preferences;
                    }
                } else if ($role === 'parent') {
    ...
    public function instructions($slug)
    {
        ...
        $attempts = DB::table('quizzes_attempts')->where('users_id', $user->id)->where('quizzes_id', $record->id)->first();

        if(!$attempts) {
            DB::table('quizzes_attempts')->insert([
                'quizzes_id' => $record->id,
                'users_id'   => $user->id,
                'attempts'   => 10
            ]);
        ...
        if (!empty($attempts)) {
            if ($attempts->attempts == 0) {
                ...
                flash('Ошибка', 'У вас не осталось попыток', 'error');
    ...
    public function finishExam(Request $request, $slug)
    {
        ...
        $attempt = DB::table('quizzes_attempts')->where('quizzes_id', $quiz->id)->where('users_id', $user_record->id)->first();

        $attempt->attempts--;

        if ($attempt->attempts == 0) {
            DB::table('quizzes_attempts')->where('quizzes_id', $quiz->id)->where('users_id', $user_record->id)->update(['attempts' => NULL]);
        }


        DB::table('quizzes_attempts')->where('quizzes_id', $quiz->id)->where('users_id', $user_record->id)->update(['attempts' => $attempt->attempts]);

        $history = DB::table('history_points')->where('exam_points', $quiz->id)->first();
        ...
        if (!$history){
            ...
            DB::table('history_points')->insert($data);
            $asd = DB::table('users')->where('id', $user_record->id)->first();

            $sum = $asd->points + $quiz->points;

            DB::table('users')->where('id', $user_record->id)->update(['points' => $sum]);
        }
        ...
        $history_quiz = DB::table('quizresults')
        ...
        if ($history_quiz) {
            if ($history_quiz->points == 0) {
                $record->points = $quiz->points;
            }
        }
        ...
        $labels_time = [getPhrase('total_time') . ' (сек.)', getPhrase('consumed_time') . ' (сек.)'];
            ...
            if (is_numeric($key)) {
                ...
                switch ($question_type) {
                    ...
                    case 'checkbox':
                        ...
                        foreach ($actual_answer as $key => $answer) {
                            $data['questions_array'][] = $answer->answer;
                        }
                        $actual_answers = $data['questions_array'];

                        foreach ($value as $answer_key => $answer_value) {
                            if(!in_array($answer_value, $actual_answers)){
                                $flag = 0;
                                break;
                            }
                        }
    ...
    public function getDatatable($slug = '')
    {
        ...
        if ($slug == 'all') {
            // ->join('quizzes_attempts', 'quizzes_id', '=', 'quizzes.id') 'quizzes_attempts.attempts',
            $cats = User::getUserSeleted('categories');

            $records = Quiz::join('quizcategories', 'quizzes.category_id', '=', 'quizcategories.id')->join('quizzes_attempts', 'quizzes_id', '=', 'quizzes.id')
                ->select(['title', 'quizzes_attempts.attempts', 'points', 'dueration', 'category', 'is_paid', 'total_marks', 'tags', 'quizzes.slug', 'quizzes.validity', 'quizzes.cost'])
                ->where('total_marks', '!=', 0)
                ->where('start_date', '<=', date('Y-m-d'))
                ->where('end_date', '>=', date('Y-m-d'))
                ->whereIn('quizzes.category_id', $cats)
                ->where('quizzes_attempts.users_id', $auth)
                ->where('quizzes_attempts.attempts', '!=', '0')
                ->get();
            ...
            $records = Quiz::join('quizcategories', 'quizzes.category_id', '=', 'quizcategories.id')->join('quizzes_attempts', 'quizzes_id', '=', 'quizzes.id')
                ->select(['title', 'quizzes_attempts.attempts', 'points', 'dueration', 'category', 'is_paid', 'total_marks', 'quizzes.slug', 'quizzes.validity', 'quizzes.cost'])
    ...
    public function examAttempts($slug, $exam_slug = '')
    {
        ...
        if (isset($exam_record->end_date)) {
            if ($date == $exam_record->end_date) {
                $data['arr'] = 1;
            } else {
                $data['arr'] = 0;
            }
    ...
    public function getExamAttemptsData($slug, $exam_slug = '')
    {
        ...
        if (!checkRole(getUserGrade(1))) {
            ...
            $auth = Auth::user();
            $settings = json_decode($auth->settings);
            $array = [];
            foreach ( $settings->user_preferences->quiz_categories as $set){
                $quiz = Quiz::where('category_id', '=', $set)->get();
                foreach ($quiz as $q) {
                    $array[] = $q->id;
                }
            }
        ...
        if (!$exam_slug) {
            if (!checkRole(getUserGrade(1))) {
                $records = Quiz::join('quizresults', 'quizzes.id', '=', 'quizresults.quiz_id')
                    ->select(['title', 'marks_obtained', 'quizresults.points', 'exam_status', 'quizresults.created_at', 'quizzes.total_marks', 'quizzes.slug', 'quizresults.slug as resultsslug', 'user_id'])
                    ->where('user_id', '=', $user->id)
                    ->whereIn('quiz_id', $array)
        ...
        return Datatables::of($records)
            ...
            ->editColumn('title', function ($records) {
                ...
                return '<a href="' . URL_RESULTS_VIEW_ANSWERS . $records->slug . '/' . $records->resultsslug . '">' . $records->title . '</a>';
            })
            ...
            ->editColumn('exam_status', function ($records) {
                ...
                return ($result == 'Pass') ? '<span class="label label-success">' . 'Прошел' . '</span>' : '<span class="label label-danger">' . 'Не прошел' . '</span>';
            })
    ...
    public function getExamAnalysisData($slug)
    {
        ...
        $records = Quiz::join('quizresults', 'quizzes.id', '=', 'quizresults.quiz_id')
            ->select(['title', 'dueration', \DB::raw('MAX(quizresults.percentage) as max'), \DB::raw('COUNT(quizresults.user_id) as attempts'), 'quizzes.slug', 'user_id'])
            ->where('user_id', '=', $user->id)
            ->groupBy('quizresults.quiz_id')
            ->get();
...
//                $date = date("Y-m-d");
//
//                if ($records->end_date >= $date){
//                    return '<span class="label label-primary"> Активный </span>';
//                }else{
//                    return '<span class="label label-danger"> Неактивный </span>';
//                }
//
//            })
            ->editColumn('title', function ($records) {
    ...
    public function overallSubjectAnalysis($slug)
    {
        ...
        $exams = DB::table('quizresults')->join('quizzes', 'quizzes.id', '=', 'quizresults.quiz_id')->where('user_id', $user->id)->get();
        ...
        if (!$records) {
            flash(getPhrase('Oops'), getPhrase('no_categories_available'), 'overlay');
            return back();
        }
    ...
    public function saveResumeExamData(Request $request)
    {
        ...
        try {
            ...
            $return_data['status'] = 'Fail';
            $return_data['message'] = $ex->getLine() . $ex->getMessage();
        }
